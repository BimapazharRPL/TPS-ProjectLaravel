...
<div class="sidebar" id="sidebar">
    ...
    <h2>Kegiatan</h2>
<a href="javascript:void(0);" class="btn btn-primary mb-3" onclick="openCreateFormQ()">+ Create New</a>
    <div class="contener" id="kegiatan">
        <table>
            <thead>
                <tr>
                    <th>No</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <img src="{{ asset('storage/' . $kegiatan->foto) }}" alt="{{ $kegiatan->judul }}" style="max-width: 100px;">
                    </td>                   
                    <td>{{ $kegiatan->keterangan }}</td>
                    <td>
                    <a href="javascript:void(0);" class="btn btn-sm btn-warning" onclick="openEditFormQ('{{ $kegiatan->id }}', '{{ $kegiatan->judul }}', '{{ $kegiatan->keterangan }}')">Edit</a>
    ...
    <!-- create -->
    <div id="createFormQ" class="modal" style="display:none">
    <div class="modal-content">
        <span class="close" onclick="closeCreateFormQ()">&times;</span>
        <!-- Isi dengan formulir sesuai kebutuhan -->
        <form method="POST" enctype="multipart/form-data" action="{{ route('kegiatan.store') }}">
            @csrf
            <label for="judul">Nama :</label>
            <input type="text" id="judul" name="judul" placeholder="masukan nama kegiatan" required>

            <label for="foto">Gambar :</label>
            <input type="file" id="foto" accept="image/*" name="foto" required>

            <label for="keterangan">Keterangan :</label>
            <textarea name="keterangan" id="keterangan" placeholder="masukan keterangan kegiatan" cols="1" rows="4" required></textarea>
    ...
    <!-- edit -->
    @foreach($kegiatans as $editkegiatan)
    <div id="editFormQ{{ $editkegiatan->id }}" class="modal" style="display:none">
    <div class="modal-content">
        <span class="close" onclick="closeEditFormQ('{{ $editkegiatan->id }}')">&times;</span>
        @if(isset($kegiatan))
        <form method="POST" enctype="multipart/form-data" action="{{ route('kegiatan.update', $editkegiatan->id ) }}">
            @csrf
            @method('PUT')
            <label for="judul">Nama :</label>
            <input type="text" id="judul" name="judul" value="{{ old('judul') ?? $editkegiatan->judul }}" required>
            ...
            <label for="keterangan">Keterangan :</label>
            <textarea name="keterangan" id="keterangan" cols="1" rows="4" required>{{ old('keterangan') ?? $editkegiatan->keterangan }}</textarea>
